import 'source-map-support/register'
import { APIGatewayProxyEvent, APIGatewayProxyHandler, APIGatewayProxyResult } from 'aws-lambda'
import { LambdaUtils } from '../LambdaUtils'
import { createLogger } from '../../utils/logger'
import { UserManager } from "../../business-logic/UserManager";
import { FriendManager } from "../../business-logic/FriendManager";
import { Message } from "../../models/messages/Message";

const logger = createLogger('getMessages')

/**
 * The get messages api handler to get the current logged in user's conversation for a particular friend
 *
 * @param event the event triggering the handler
 * @author: Ulhas Pai
 */
export const handler: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    logger.info("Started")

    // grab the user id from the JWT payload
    const userId: string = LambdaUtils.getUserId(event)
    logger.info("userId = " + JSON.stringify(userId))

    // | (pipes) are a problem, so client will encode and send the friendId
    const friendId: string = decodeURIComponent(event.pathParameters.friendId)
    const fromTimestamp: string = decodeURIComponent(event.pathParameters.from)
    const toTimestamp: string = decodeURIComponent(event.pathParameters.to)

    let messages: Message<any>[] = [];
    if (await FriendManager.isAFriend(userId, friendId)) {
        // TODO: validate fromTimestamp and toTimestamp
        try {
            // get messages
            messages = await UserManager.getMessages(userId, friendId, new Date(fromTimestamp), new Date(toTimestamp));
        } catch (err) {
            // thrown if there are no messages and no index
            logger.info("Error: " + JSON.stringify(err))
        }
    }

    return {
        statusCode: 200,
        headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Credentials': true
        },
        body: JSON.stringify({
            items: messages
        })
    }
}
